...
/*            if (repeat > 0) {                                   // If we have any repeat on, change the coordinates to their "local" repetitions
        ...
        public static double grad(int hash, double x, double y, double z) {
            ...
            double u = h < 8  ? x : y;              // If the most signifigant bit (MSB) of the hash is 0 then set u = x.  Otherwise y.

            double v;                                           // In Ken Perlin's original implementation this was another conditional operator (?:).  I
                                                                // expanded it for readability.

            if (h < 4 )                             // If the first and second signifigant bits are 0 set v = y
                v = y;
            else if (h == 12 || h == 14 )// If the first and second signifigant bits are 1 set v = x
                v = x;
