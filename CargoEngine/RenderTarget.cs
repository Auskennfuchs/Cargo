...
namespace CargoEngine {
    public class RenderTarget {
        ...
        public RenderTarget(Renderer renderer, Resource res) {
            this.renderer = renderer;
            ReCreate(res);
        }

        public RenderTarget(Renderer renderer,int width, int height, Format format) {
            this.renderer = renderer;
            var t = new Texture2D(renderer.Device, new Texture2DDescription() {
                ArraySize = 1,
        ...
        public void ReCreate(Resource res) {
            Dispose();
            View = new RenderTargetView(renderer.Device, res);
...
/*        public void Activate(Renderer pipeline) {
        ...
        public void AddDepthStencil() {
            ...
            using (var backBuffer = View.ResourceAs<Texture2D>()) {
                ...
                using (var depthBuffer = new Texture2D(renderer.Device, depthBufferDescription)) {
                    DepthStencilState = new DepthStencilState(renderer.Device, depthStencilStateDescription);
                    DepthStencilView = new DepthStencilView(renderer.Device, depthBuffer, depthStencilViewDescription);
                }
            }
        }
    }
